...
#-------------------------------------------------------------------------
# Helpers for unknown sigma/mu (Stage 2)
# left hemisphere = populations 1..N/2, right hemisphere = populations N/2+1..N
#-------------------------------------------------------------------------

build_cm<-function(N,Gamma,h,sigL,sigR){
  sig_pop<-c(rep(sigL,N/2),rep(sigR,N/2))
  sig_vec<-c()
  for(j in 1:N){ sig_vec<-c(sig_vec,c(1,sig_pop[j],1)) }
  Sigma<-diag(sig_vec,3*N,3*N)
  return(chol(covariance_matrix_JRNMM(N,Gamma,Sigma,h)))
}

mu_pop<-function(N,muL,muR){
  return(c(rep(muL,N/2),rep(muR,N/2)))
}
...
ABC_pilot<-function(N,T,h,grid,startv,Theta,Rho,K,dm,meanVec,Gamma,summaries_X,summaries_parameters,summaries_weights,Pr_cont){
  ...
  #sigmas and mus unknown
  sigL<-runif(1,Pr_cont[N+3,1],Pr_cont[N+3,2])
  sigR<-runif(1,Pr_cont[N+4,1],Pr_cont[N+4,2])
  muL <-runif(1,Pr_cont[N+5,1],Pr_cont[N+5,2])
  muR <-runif(1,Pr_cont[N+6,1],Pr_cont[N+6,2])
  Theta[,6]<-mu_pop(N,muL,muR)
  cm<-build_cm(N,Gamma,h,sigL,sigR)
  for(j in 1:(N-1)){
    for(k in (j+1):N){
...
ABC_SMC_r1<-function(N,T,h,grid,startv,Theta,Rho,K,dm,meanVec,Gamma,summaries_X,summaries_parameters,summaries_weights,Pr_cont,delta_1){
  ...
  ret<-array(0,dim=c(1,dim_arr,1))
  ret[,,1]<-c(Dist,A_vector,L,c,sigL,sigR,muL,muR,Rho_vec2)  
...
ABC_SMC<-function(N,T,h,grid,startv,Theta,Rho,K,dm,meanVec,Gamma,summaries_X,summaries_parameters,summaries_weights,kept_A,kept_L,kept_c,kept_sigL,kept_sigR,kept_muL,kept_muR,kept_weights_c,sigma_kernel,hat_p_vec,stay_prob,Pr_cont,delta_r){
  ...
  while(Dist>=delta_r){
    ...
    L<-kept_L[index_c] 
    c<-kept_c[index_c] 
    theta_c_sampled<-c(As,L,c,kept_sigL[index_c],kept_sigR[index_c],kept_muL[index_c],kept_muR[index_c])
    ...
    sigL<-theta_c_perturbed[N+3]
    sigR<-theta_c_perturbed[N+4]
    muL <-theta_c_perturbed[N+5]
    muR <-theta_c_perturbed[N+6]
  ...
  ret<-array(0,dim=c(1,dim_arr,1))
  ret[,,1]<-c(Dist,As,L,c,sigL,sigR,muL,muR,Rho_vec2,count_acc)  
