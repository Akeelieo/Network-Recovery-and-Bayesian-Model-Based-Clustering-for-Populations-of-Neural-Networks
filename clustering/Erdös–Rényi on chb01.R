...
MCMC_mix_err_sbm<-function(C,B,iter,burn_in,n_nodes,N_data,G_data,prob_vec,a_0,b_0,c_0,d_0,e_0,f_0,z_init,c_init,p_init,q_init,theta_init,perturb,rw_steps,tau_conc=1){ 
  # tau_conc: Dirichlet concentration for the cluster-weight prior. Default 1
  #   reproduces the original behaviour (rdirichlet(1,1+h)). Larger values
  #   (e.g. 5, 10) penalise lopsided clusters and discourage the 1-vs-N collapse.
  ...
  for (i in 1:iter){
    ...
    taus_current <- rdirichlet(1,tau_conc+h) #update taus using z_current (tau_conc: balance prior)
    ...
    for (c in 1:C){
      ...
      # --- ER case (B==1): every node is in the single block, nothing to sample.
      #     This makes the representative Erdos-Renyi: one shared theta per cluster.
      if (B==1){
        c_current[[c]][] <- 1
      } else {
