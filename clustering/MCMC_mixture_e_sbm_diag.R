...
MCMC_mix_err_sbm<-function(C,B,iter,burn_in,n_nodes,N_data,G_data,prob_vec,a_0,b_0,c_0,d_0,e_0,f_0,z_init,c_init,p_init,q_init,theta_init,perturb,rw_steps,psi=1){ 
  ...
  # psi: symmetric Dirichlet concentration for the cluster-weight (tau) prior.
  #      psi=1 (default) reproduces the ORIGINAL flat prior rdirichlet(1,1+h).
  #      psi>1 favours BALANCED clusters (pushes tau toward 1/C each) and
  #      penalises lone-outlier clusters; psi<1 favours lopsided/sparse splits.
  ...
  # <- which(upper.tri(toy_mat),arr.ind = TRUE) # gives the positions of the upper triangle elements in matrix of size n_nodes, i.e. the pairs of nodes(possible edges) for undirected networks
  ...
  # ---- ACCEPTANCE COUNTERS (diagnostic instrumentation) --------------
  # We detect acceptance by whether the returned value changed. This needs
  # no edits to MH_updates.R. "prop" counts how many times each move TYPE
  # was actually proposed (its branch fired); "acc" counts changes.
  `%||%` <- function(a, b) if (is.null(a) || is.na(a)) b else a   # for the printf
  acc_rep <- 0; prop_rep <- 0   # local-perturbation representative move (v[1:5])
  acc_out <- 0; prop_out <- 0   # independence-sampler representative move (v[6])
  acc_p   <- 0; prop_p   <- 0
  acc_q   <- 0; prop_q   <- 0
  for (i in 1:iter){
    print(c("iter: ",i))
    ...
    taus_current <- rdirichlet(1,psi+h) #update taus using z_current (psi = symmetric Dirichlet concentration; psi=1 is the original flat prior)
    ...
    for (c in 1:C){
      ...
      if (1 %in% v[1:5]){ # Update representative with one of the 6 possible proposals
        ...
        prop_rep <- prop_rep + 1                                        # DIAG
        if (any(.G_before != G_m_current[[c]])) acc_rep <- acc_rep + 1  # DIAG
      }
      ...
      if (v[6]==1){
        ...
        prob <- min(1, mhr)
        gen <- rbinom(1, 1, prob)
        prop_out <- prop_out + 1                 # DIAG
        if (gen==1){
          G_m_current[[c]] <- G_m_new
          acc_out <- acc_out + 1                 # DIAG
        }
      }
      ...
      if(1 %in% v[7:12]){ 
        ...
        prop_p <- prop_p + 1                                         # DIAG
        if (.p_before != p_current[c]) acc_p <- acc_p + 1            # DIAG
      }
      
      # MH Step: Update q_c
      if(1 %in% v[13:18]){ 
        ind_cl <- which(z_current==c) #find which graphs belong to c class, gives their index to identify them in list of G_data
        .q_before <- q_current[c]                                    # DIAG
        q_current[c] <- MH_update_q(p_current[c],rw_steps[which(v==1)-12],G_m_current[[c]],q_current[c],G_data,ind_cl,theta_current[[c]],
                                    c_current[[c]],n_nodes,lab,a_0[[c]],b_0[[c]],c_0[[c]],d_0[[c]])
        prop_q <- prop_q + 1                                         # DIAG
        if (.q_before != q_current[c]) acc_q <- acc_q + 1            # DIAG
      }
      ...
      for(n in 1:n_nodes){
        ...
        j_nodes <- c(setdiff(1:n_nodes, n), setdiff(1:n_nodes, n))        
  ...
  # ---- ACCEPTANCE REPORT ---------------------------------------------
  safe <- function(a, p) if (p > 0) a / p else NA_real_
  accept <- list(
    rep_local = safe(acc_rep, prop_rep),  # local edge-perturbation rep move
    rep_indep = safe(acc_out, prop_out),  # independence-sampler rep move
    p         = safe(acc_p,   prop_p),
    q         = safe(acc_q,   prop_q),
    counts    = c(prop_rep=prop_rep, prop_out=prop_out, prop_p=prop_p, prop_q=prop_q)
  )
  cat(sprintf("\n[ACCEPT] rep_local=%.3f (n=%d)  rep_indep=%.3f (n=%d)  p=%.3f (n=%d)  q=%.3f (n=%d)\n",
              accept$rep_local %||% NA, prop_rep, accept$rep_indep %||% NA, prop_out,
              accept$p %||% NA, prop_p, accept$q %||% NA, prop_q))
  cat("[ACCEPT] Healthy random-walk target ~0.2-0.4. Near 0 = proposals rejected\n")
  cat("[ACCEPT] (posterior too peaked / steps too big); near 1 = steps too small.\n")

  return(list(chain_taus,chain_z,chain_p,chain_q,chain_G_m,chain_c,chain_w,chain_theta,accept)) 
}
